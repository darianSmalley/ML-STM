...
Modifyied by Darain Smalley (email: darian.smalley@gmail.com)
"""
...
class MakeAtom:
    ...
    def atom2dgaussian(self) -> np.ndarray:
        ...
        g = self.offset + self.intensity * np.exp(
            -(
                a * ((self.x - self.xo) ** 2)
                + 2 * b * (self.x - self.xo) * (self.y - self.yo)
                + c * ((self.y - self.yo) ** 2)
            )
        )
        return g
    ...
    def gen_atom_mask(self) -> Tuple[np.ndarray]:
        ...
        mask = mask[
            np.min(np.where(mask > 0)[0]) : np.max(np.where(mask > 0)[0] + 1),
            np.min(np.where(mask > 0)[1]) : np.max(np.where(mask > 0)[1]) + 1,
        ]
        mask[mask > 0] = 1
...
def create_lattice_mask(
    ...
    **kwargs: int
    ...
    lattice_mask = np.zeros_like(lattice, dtype=float)
    gauss_mask = np.zeros_like(lattice, dtype=float)
    w, h = lattice_mask.shape
    for xy in xy_atoms:
        x, y = xy
        x = int(np.around(x))
        y = int(np.around(y))
        gaussians, mask = create_mask_func(scale, rmask)
        mw, mh = mask.shape
        r_m = mw / 2
        r_m1 = int(r_m + 0.5)
        r_m2 = int(r_m - 0.5)
        ...
        if left < 0:
            cut = -left
            mask = np.delete(mask, np.s_[0:cut], axis=0)
            gaussians = np.delete(gaussians, np.s_[0:cut], axis=0)
            left = 0

        if right > w:
            cut = right - w
            mask = np.delete(mask, np.s_[mw - cut : mw], axis=0)
            gaussians = np.delete(gaussians, np.s_[mw - cut : mw], axis=0)
            right = w

        if up > h:
            cut = up - h
            mask = np.delete(mask, np.s_[mh - cut : mh], axis=1)
            gaussians = np.delete(gaussians, np.s_[mh - cut : mh], axis=1)
            up = h

        if down < 0:
            cut = -down
            mask = np.delete(mask, np.s_[0:cut], axis=1)
            gaussians = np.delete(gaussians, np.s_[0:cut], axis=1)
            down = 0

        prev = lattice_mask[left:right, down:up]
        lattice_mask[left:right, down:up] = prev + mask

        prev_g = gauss_mask[left:right, down:up]
        gauss_mask[left:right, down:up] = prev_g + gaussians

    return gauss_mask, lattice_mask


def create_multiclass_lattice_mask(
    imgdata: np.ndarray,
    ...
    **kwargs: int
    ...
    for i, img in enumerate(imgdata):
        gauss_mask, lattice_mask = create_multiclass_lattice_mask_(
            img, coord_class_dict[i], *args, **kwargs
        )
        masks.append(lattice_mask)
        guassians.append(gauss_mask)
    shapes = [m.shape for m in masks]
    if len(set(shapes)) <= 1:
        masks = np.array(masks)
        guassians = np.array(guassians)

    return guassians, masks


def create_multiclass_lattice_mask_(
    lattice: np.ndarray,
    xyz_atoms: np.ndarray,
    *args: Callable[[int, int], Tuple[np.ndarray, np.ndarray]],
    **kwargs: int
) -> np.ndarray:
    """
    ...
    for atom in xyz_atoms:
        ...
        x = int(np.around(x))
        y = int(np.around(y))
        gaussians, mask = create_mask_func(scale, rmask, z)
        mw, mh = mask.shape
        r_m = mw / 2
        r_m1 = int(r_m + 0.5)
        r_m2 = int(r_m - 0.5)
        ...
        prev = lattice_mask[left:right, down:up, atom_ch_d[z]]
        lattice_mask[left:right, down:up, atom_ch_d[z]] = prev + mask

        prev_g = gauss_mask[left:right, down:up, atom_ch_d[z]]
        gauss_mask[left:right, down:up, atom_ch_d[z]] = prev_g + gaussians
